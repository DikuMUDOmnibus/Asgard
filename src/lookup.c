...
int flag_lookup(const char *name, const struct flag_type *flag_table)
{
	...
	{
		if (LOWER(name[0]) == LOWER(flag_table[flag].name[0]) && !str_prefix(
				name, flag_table[flag].name))
...
int sex_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(sex_table[sex].name[0]) && !str_prefix(
				name, sex_table[sex].name))
			return sex;
	}
...
int size_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(size_table[size].name[0]) && !str_prefix(
				name, size_table[size].name))
			return size;
	}
