...
bool is_friend(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if (IS_SET(ch->off_flags,ASSIST_VNUM) && ch->pIndexData
			== victim->pIndexData)
		return TRUE;
	...
	if (IS_SET(ch->off_flags,ASSIST_ALIGN) && !IS_SET(ch->act,ACT_NOALIGN)
			&& !IS_SET(victim->act,ACT_NOALIGN) && ((IS_GOOD(ch)
			&& IS_GOOD(victim)) || (IS_EVIL(ch) && IS_EVIL(victim))
...
int race_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(race_table[race].name[0]) && !str_prefix(
				name, race_table[race].name))
			return race;
	}
...
int liq_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(liq_table[liq].liq_name[0]) && !str_prefix(
				name, liq_table[liq].liq_name))
			return liq;
	}
...
int weapon_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(weapon_table[type].name[0]) && !str_prefix(
				name, weapon_table[type].name))
			return type;
	}
...
int item_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(item_table[type].name[0]) && !str_prefix(
				name, item_table[type].name))
...
int attack_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(attack_table[att].name[0]) && !str_prefix(
				name, attack_table[att].name))
			return att;
	}
...
long wiznet_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(wiznet_table[flag].name[0]) && !str_prefix(
				name, wiznet_table[flag].name))
			return flag;
	}
...
int class_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(class_table[class].name[0]) && !str_prefix(
				name, class_table[class].name))
			return class;
	}
...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	{
		printf_debug("Bad sn %d in get_skill.", sn);
		skill = 0;
#ifdef PUCK_DEBUG
		if (wears_debug(ch))
		{
	...
	else /* mobiles */
	{
		...
		else if (sn == gsn_hide)
		{
		...
		else if ((sn == gsn_dodge && IS_SET(ch->off_flags,OFF_DODGE)) || (sn
				== gsn_phase && IS_SET(ch->off_flags,OFF_PHASE)) || (sn
				== gsn_parry && IS_SET(ch->off_flags,OFF_PARRY)))
			skill = ch->level;

		else if (sn == gsn_shield_block)
			skill = 10 + ch->level;

		else if (sn == gsn_blood_phase)
			skill = 10 + ch->level;

		else if (sn == gsn_shield_wall)
			skill = 10 + ch->level;
			...
			skill = 10 + ch->level;
				...
				|| IS_SET(ch->act,ACT_THIEF) || IS_SET(ch->act,ACT_RANGER)
		...
		else if (sn == gsn_sword || sn == gsn_dagger || sn == gsn_spear || sn
				== gsn_mace || sn == gsn_axe || sn == gsn_flail || sn
				== gsn_whip || sn == gsn_polearm)
...
int check_weapon_handling(CHAR_DATA *ch, sh_int sn)
{
	...
	if (has_skill(ch, wsn))
	{
		...
		skill = ch->pcdata->learned[wsn] / 10; // Back to 10%!
		return skill;
	}
...
void reset_char(CHAR_DATA *ch)
{
	...
	check_focus(ch); // reset perm_hit/mana/move values!
...
//	ch->move = ch->max_move;
	...
	if (ch->pcdata->focus[MAGIC_DEFENSE])
		ch->saving_throw -= focus_save(ch,0); //focus_level(ch->pcdata->focus[MAGIC_DEFENSE]);
...
void affect_modify(CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd)
{
	...
	{
	default:
		printf_debug("Affect_modify: unknown location %d.", paf->location);
		return;
...
void affect_remove(CHAR_DATA *ch, AFFECT_DATA *paf)
{
	...
	if (ch->affected == NULL)
	{
		printf_debug("Affect_remove: no affect.");
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Affect_remove: cannot find paf.");
			return;
		}
	}
...
void cooldown_remove(CHAR_DATA *ch, COOLDOWN_DATA *pcd)
{
	...
	{
		// printf_debug("Cooldown_remove: no cooldowns."); Bug line removed due to log flooding 
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Cooldown_remove: cannot find pcd.");
			return;
		}
	}
...
void affect_remove_obj(OBJ_DATA *obj, AFFECT_DATA *paf)
{
	...
	{
		printf_debug("Affect_remove_object: no affect.");
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Affect_remove_object: cannot find paf.");
			return;
		}
	}
...
void affect_remove_room(ROOM_INDEX_DATA *room, AFFECT_DATA *paf)
{
	...
	{
		printf_debug("Affect_remove_room: no affect.");
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Affect_remove_room: cannot find paf.");
			return;
		}
	}
...
void char_from_room(CHAR_DATA *ch)
{
	...
	if (ch->in_room == NULL)
	{
		printf_debug("Char_from_room: NULL.");
		return;
	}
	...
	if ((obj = get_eq_char(ch, WEAR_LIGHT )) != NULL && obj->item_type
			== ITEM_LIGHT && obj->value[2] != 0 && ch->in_room->light > 0)
		--ch->in_room->light;
	...
	else
	{
		...
		if (prev == NULL)
			printf_debug("Char_from_room: ch not found.");
	}
...
void char_to_room(CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex)
{
	...
	if (pRoomIndex == NULL)
	{
		...
		printf_debug("Char_to_room: NULL.");
...
void obj_from_char(OBJ_DATA *obj)
{
	...
	{
		printf_debug("Obj_from_char: null ch.");
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
			printf_debug("Obj_from_char: obj not in list.");
	}
	obj->carried_by = NULL;
...
int apply_ac(int level, OBJ_DATA *obj, int iWear, int type)
{
	...
	case WEAR_ARMS:
		return obj->value[type];
	case WEAR_SHIELD:
		return (obj->value[type] * 5) / 2; // raised to 2.5
	case WEAR_FINGER_L:
		return obj->value[type];
	case WEAR_FINGER_R:
		return obj->value[type];
...
void equip_char(CHAR_DATA *ch, OBJ_DATA *obj, int iWear)
{
	...
	{
		printf_debug("Equip_char: already equipped (%d).", iWear);
		return;
	}
	...
	if (ch->level < LEVEL_IMMORTAL)
	{
		if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch))
				...
				|| (!IS_NPC(ch) && !clan_can_use(ch, obj)) || (!IS_NPC(ch)
				&& !class_can_use(ch, obj)) || !is_class_restricted(ch, obj))
		{
			/*
	...
	if (IS_OBJ_STAT(obj,ITEM_QUESTPOINT) && !IS_NPC(ch))
	{
		...
		ch->questpoints += 300;
		act("{YYou gained 300 {RQuest Point {Yfrom $p!{x", ch, obj, NULL, TO_CHAR);
		act("{Y$n gained 300 {RQuest Point {Yfrom $p!{x", ch, obj, NULL, TO_ROOM);
	}
	...
	if (!obj->enchanted)
		for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
			if (paf->location != APPLY_SPELL_AFFECT && paf->location
					!= APPLY_MORPH_FORM)
				affect_modify(ch, paf, TRUE );
	for (paf = obj->affected; paf != NULL; paf = paf->next)
		if (paf->location == APPLY_SPELL_AFFECT || paf->location
				== APPLY_MORPH_FORM) {
			if (paf->location == APPLY_MORPH_FORM) {
			    // Hack to ensure morph randoms give right gsn.
			    paf->type = gsn_conceal;
			}
			affect_to_char(ch, paf);
		}else
			affect_modify(ch, paf, TRUE );
...
void unequip_char(CHAR_DATA *ch, OBJ_DATA *obj)
{
	...
	if (obj->wear_loc == WEAR_NONE)
	{
		printf_debug("Unequip_char: already unequipped.");
		return;
	}
	...
	if (!obj->enchanted)
	{
		for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
			...
			{
				if (paf->location == APPLY_MORPH_FORM)
					affect_strip(ch, gsn_conceal);	// paf->type changed to gsn_conceal.
				for (lpaf = ch->affected; lpaf != NULL; lpaf = lpaf_next)
				{
					lpaf_next = lpaf->next;
					if ((lpaf->type == paf->type)
							&& (lpaf->level == paf->level) && (lpaf->location
							== APPLY_SPELL_AFFECT))
					{
	...
	for (paf = obj->affected; paf != NULL; paf = paf->next)
				...
				== APPLY_MORPH_FORM)
		{
			...
			for (lpaf = ch->affected; lpaf != NULL; lpaf = lpaf_next)
			{
				...
				{
					printf_debug("location = %d", lpaf->location);
					printf_debug("type = %d", lpaf->type);
					affect_remove(ch, lpaf);
					lpaf_next = NULL;
				}
			}
...
void obj_from_room(OBJ_DATA *obj)
{
	...
	{
		printf_debug("obj_from_room: NULL.");
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Obj_from_room: obj not found.");
			return;
		}
	}
...
void obj_to_room(OBJ_DATA *obj, ROOM_INDEX_DATA *pRoomIndex)
{
	...
	if (pRoomIndex == NULL)
	{
		printf_debug("OBJ_TO_ROOM: pRoomIndex NULL POINTER");
		return;
	}
...
void obj_to_obj(OBJ_DATA *obj, OBJ_DATA *obj_to)
{
	if (obj == NULL || obj_to == NULL) {
		printf_debug("obj_to_obj: NULL obj or obj_to");
		return;
	}
...
void obj_from_obj(OBJ_DATA *obj)
{
	...
	{
		printf_debug("Obj_from_obj: null obj_from.");
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Obj_from_obj: obj not found.");
			return;
		}
	}
...
void extract_obj(OBJ_DATA *obj)
{
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Extract_obj: obj %d not found.", obj->pIndexData->vnum);
			return;
		}
	}
...
void extract_char(CHAR_DATA *ch, bool fPull)
{
	...
	if (ch->in_room == NULL)
	{
		printf_debug("Extract_char: NULL. Attempting fix");
	...
	if ((!fPull) && (IS_SET(ch->in_room->room_flags,ROOM_ARENA)))
	{
		char_from_room(ch);
	...
	if (!fPull)
	{
		char_to_room(ch, get_clan_deathroom(ch));
		return;
	}
	...
	else
	{
		...
		if (prev == NULL)
		{
			printf_debug("Extract_char: char not found.");
			return;
		}
	}
...
CHAR_DATA *get_char_room(CHAR_DATA *ch, char *argument)
{
	...
	for (rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room)
	{
		...
		{
			if (rch->morph_form[0] >= 5 && rch->morph_form[0] <= 9 && !str_cmp(
					arg, "dragon"))
			{
				if (++count == number)
					return rch;
	...
	for (wch = char_list; wch != NULL; wch = wch->next)
	{
		...
		for (rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room)
		{
			...
			if (rch->morph_form[0] != 0)
			{
				...
				else if (rch->morph_form[0] == MORPH_BAT
						&& !str_cmp(arg, "bat"))
				{
					if (++count == number)
						return rch;
				}
				else if (rch->morph_form[0] == MORPH_WOLF && !str_cmp(arg,
						"wolf"))
				{
					if (++count == number)
						return rch;
				}
				else if (rch->morph_form[0] == MORPH_BEAR && !str_cmp(arg,
						"bear"))
				{
					if (++count == number)
						return rch;
...
CHAR_DATA *get_char_world(CHAR_DATA *ch, char *argument)
{
	...
	for (wch = char_list; wch != NULL; wch = wch->next)
	{
		if (wch->in_room == NULL || !can_see(ch, wch) || !is_name(arg,
				wch->name))
			continue;
		if (++count == number)
			return wch;
...
CHAR_DATA *get_char_mortal(CHAR_DATA *ch, char *argument)
{
	...
	for (wch = char_list; wch != NULL; wch = wch->next)
	{
		if (!str_cmp(arg, wch->name) && !IS_NPC(wch) && !IS_IMMORTAL(wch))
			return wch;
	}
...
OBJ_DATA *get_obj_carry(CHAR_DATA *ch, char *argument)
{
	...
	for (obj = ch->carrying; obj != NULL; obj = obj->next_content)
	{
		if (obj->wear_loc == WEAR_NONE && (can_see_obj(ch, obj)) && is_name(
				arg, obj->name))
		{
			if (++count == number)
				return obj;
...
OBJ_DATA *get_obj_wear(CHAR_DATA *ch, char *argument)
{
	...
	for (obj = ch->carrying; obj != NULL; obj = obj->next_content)
	{
		if (obj->wear_loc != WEAR_NONE && can_see_obj(ch, obj) && is_name(arg,
				obj->name))
		{
			if (++count == number)
				return obj;
...
void deduct_cost(CHAR_DATA *ch, int cost, int value)
{
	...
	case VALUE_PLATINUM:
		...
		if (ch->platinum < 0)
		{
			printf_debug("deduct costs: platinum %d < 0", ch->platinum);
			ch->platinum = 0;
		}
		break;
	...
	case VALUE_GOLD:
		...
		if (ch->gold < 0)
		{
			printf_debug("deduct costs: gold %d < 0", ch->gold);
			ch->gold = 0;
		}
		break;
	...
	case VALUE_SILVER:
		...
		if (ch->silver < 0)
		{
			printf_debug("deduct costs: silver %d < 0", ch->silver);
			ch->silver = 0;
		}
		break;
	...
	default:
		printf_debug("deduct costs: invalid value.");
		break;
	}
}
...
OBJ_DATA *create_money(int platinum, int gold, int silver)
{
	...
	if (platinum < 0 || gold < 0 || silver < 0 || (platinum == 0 && gold == 0
			&& silver == 0))
	{
		printf_debug( "Create_money: zero or negative money.",
...
bool room_is_dark(ROOM_INDEX_DATA *pRoomIndex)
{
	...
	if (pRoomIndex->sector_type == SECT_INSIDE || pRoomIndex->sector_type
			== SECT_CITY)
		return FALSE;
...
{
	if (IS_SET(pRoomIndex->room_flags, ROOM_IMP_ONLY) && get_trust(ch)
			< MAX_LEVEL)
		return FALSE;
	...
	if (IS_SET(pRoomIndex->room_flags,ROOM_NEWBIES_ONLY) && ch->level > 20
			&& !IS_HERO(ch))
		return FALSE;

    // Clan stuff
    CLAN_DATA *clan;
    if (!IS_NPC(ch) && (clan = get_clan_by_vnum(pRoomIndex->vnum)) != NULL) {
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if (victim == NULL)
	{
		...
		sprintf(
				buf,
				"handler.c: function can_see(): ch=%s, room=%s, victim is NULL!!\n\r",
	...
	|| (IS_NPC(ch) && ch->pIndexData->see_all_mortals && !IS_NPC(victim) && !IS_IMMORTAL(victim)))
		return TRUE;
    ...
    if (is_same_clan(ch, victim) && !is_indep(ch))
		return TRUE;
	...
	/* sneaking */
	if (IS_AFFECTED(victim, AFF_SNEAK) && !IS_AFFECTED(ch,AFF_DETECT_MOTION)
	...
	if (is_affected(victim, gsn_shadow_blend))
	{
		if (IS_AFFECTED(ch,AFF_DETECT_MOTION))
		{
			if (!IS_NPC(victim))
				chance = get_skill(victim, gsn_shadow_blend) * 1.3;
			else
				chance = 80;

			chance = UMAX(chance,55);
...
*/
	...
	if (IS_SET(obj->extra_flags, ITEM_DARK) && !IS_AFFECTED(ch, AFF_INFRARED))
		return FALSE;
...
char *item_type_name(OBJ_DATA *obj)
{
	...
	printf_debug("Item_type_name: unknown type %d.", obj->item_type);
	return "(unknown)";
}
...
char *affect_bit_name(int vector)
{
	...
	if (vector & AFF_DETECT_MOTION)
		strcat(buf, " detect_motion");
	...
	//if ( vector & AFF_INSOMNIA      ) strcat( buf, " insomnia"      );
...
char *ability_lich_name(int ability_lich_flags)
{
	...
	buf[0] = '\0';
	if (ability_lich_flags & ABILITY_LICH_NIGHTSTALKER)
		strcat(buf, " Nightstalker");
	return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *act_bit_name(int act_flags)
{
	...
	if (IS_SET(act_flags,ACT_IS_NPC))
	{
		...
		//	if (act_flags & ACT_STOCKMAN    ) strcat(buf, " stockman");
	...
	else
	{
		...
		if (act_flags & PLR_NOGOTO)
			strcat(buf, " no_goto");
		if (act_flags & PLR_NOVIOLATE)
			strcat(buf, " no_violate");
		...
		if (act_flags & PLR_TWIT)
			strcat(buf, " twit");
		if (act_flags & PLR_NOEVOLVE)
			strcat(buf, " no_evolve");
	}
	return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *comm_bit_name(int comm_flags)
{
	...
	if (comm_flags & COMM_NOARENA)
		strcat(buf, " no_arena");
	if (comm_flags & COMM_NOPUBCHAN)
		strcat(buf, " no_pubchans");
	if (comm_flags & COMM_NOCLAN)
		strcat(buf, " no_clan");
	if (comm_flags & COMM_NOQGOSSIP)
		strcat(buf, " no_qgossip");
	if (comm_flags & PLR_NOCGOSSIP)
		strcat(buf, " no_cgossip");
	if (comm_flags & SHD_NOPRAY)
		strcat(buf, " no_pray");
	if (comm_flags & COMM_NOASK)
		strcat(buf, " no_ask");
	if (comm_flags & COMM_NOMUSIC)
		strcat(buf, " no_music");
	if (comm_flags & COMM_NOCHAT)
		strcat(buf, " no_chat");
	if (comm_flags & COMM_COMPACT)
		strcat(buf, " compact");
	if (comm_flags & COMM_BRIEF)
		strcat(buf, " brief");
	if (comm_flags & PLR_SHORT_COMBAT)
		strcat(buf, " shortcmbt");
...
char *wear_bit_name(int wear_flags)
{
	...
	if (wear_flags & ITEM_WEAR_ANKLE)
		strcat(buf, " ankle");
	if (wear_flags & ITEM_WEAR_CLAN)
		strcat(buf, " c_patch");
	if (wear_flags & ITEM_WEAR_RELIG)
		strcat(buf, " r_patch");
	if (wear_flags & ITEM_WEAR_PATCH)
		strcat(buf, " patch");
...
char *room_bit_name(int room_flags)
{
	...
	if (room_flags & ROOM_ARENA)
		strcat(buf, " arena");
	if (room_flags & ROOM_NO_MAGIC)
		strcat(buf, " no_magic");
	if (room_flags & ROOM_SHOP)
		strcat(buf, " shop");
	if (room_flags & ROOM_REMORT)
		strcat(buf, " remort");
	if (room_flags & ROOM_ON_FIRE)
		strcat(buf, " on_fire");
...
char *weapon_bit_name(int weapon_flags)
{
	...
	if (weapon_flags & WEAPON_POISON)
		strcat(buf, " poison");
	if (weapon_flags & WEAPON_OSMOSIS)
		strcat(buf, " osmosis");
	if (weapon_flags & WEAPON_ENERGYDRAIN)
		strcat(buf, " energydrain");
...
char *sector_bit_name(int sect_flags)
{
	...
	if (sect_flags & SECT_FOREST)
		strcat(buf, " forest");
	if (sect_flags & SECT_INSIDE)
		strcat(buf, " inside");
	if (sect_flags & SECT_CITY)
		strcat(buf, " city");
	if (sect_flags & SECT_FIELD)
		strcat(buf, " field");
	if (sect_flags & SECT_HILLS)
		strcat(buf, " hills");
	if (sect_flags & SECT_MOUNTAIN)
		strcat(buf, " mountain");
	if (sect_flags & SECT_WATER_SWIM)
		strcat(buf, " water_swim");
	if (sect_flags & SECT_AIR)
		strcat(buf, " air");
	if (sect_flags & SECT_UNUSED)
		strcat(buf, " exotic");
	if (sect_flags & SECT_DESERT)
		strcat(buf, " desert");
	if (sect_flags & SECT_WATER_NOSWIM)
		strcat(buf, " noswim");
...
bool remove_voodoo(CHAR_DATA *ch)
{
	...
	for (d = descriptor_list; d != NULL; d = d->next)
	{
		...
		for (object = victim->carrying; object != NULL; object
				= object->next_content)
		{
...
int god_lookup(const char *name)
{
	...
	{
		if (LOWER(name[0]) == LOWER(god_table[god].name[0]) && !str_prefix(
				name, god_table[god].name))
			return god;
	}
...
bool wears_debug(CHAR_DATA *ch)
{
	...
	for (obj = ch->carrying; obj != NULL; obj = obj->next_content)
	{
		if ((obj->pIndexData->vnum == OBJ_VNUM_DEBUGPATCH) && (obj->wear_loc
				!= -1))
			return TRUE;
	}
...
char *get_pers(CHAR_DATA *ch, CHAR_DATA *looker)
{
	...
	{
		if ((is_affected(ch, gsn_conceal)
		||  is_affected(ch, gsn_conceal2)
		||  is_affected(ch, gsn_concealClan1)
		||  is_affected(ch, gsn_concealClan2))
		&&  is_same_clan(ch, looker))
		{
	...
	||  is_affected(ch, gsn_concealClan1)
	||  is_affected(ch, gsn_concealClan2)
	||  is_affected(ch, gsn_shadow_dance))
	&&	ch->morph_form[0] != 0)
...
bool is_class_restricted(CHAR_DATA *ch, OBJ_DATA *obj)
{
	...
	if ((restricts & WIZARD_ONLY) && ch->class != CLASS_WIZARD && ch->class
			!= CLASS_MAGE)
		canwear = FALSE;
	if ((restricts & GLADIATOR_ONLY) && ch->class != CLASS_GLADIATOR
			&& ch->class != CLASS_WARRIOR)
		canwear = FALSE;
	if ((restricts & PRIEST_ONLY) && ch->class != CLASS_PRIEST && ch->class
			!= CLASS_CLERIC)
		canwear = FALSE;
	if ((restricts & MERCENARY_ONLY) && ch->class != CLASS_MERCENARY
			&& ch->class != CLASS_THIEF)
		canwear = FALSE;
	if ((restricts & STRIDER_ONLY) && ch->class != CLASS_STRIDER && ch->class
			!= CLASS_RANGER)
		canwear = FALSE;
	if ((restricts & SAGE_ONLY) && ch->class != CLASS_SAGE && ch->class
			!= CLASS_DRUID)
		canwear = FALSE;
	if ((restricts & LICH_ONLY) && ch->class != CLASS_LICH && ch->class
			!= CLASS_SHADE)
		canwear = FALSE;
	...
	if ((restricts & ARCHMAGE_ONLY) && ch->class != CLASS_ARCHMAGE)
		canwear = FALSE;
	if ((restricts & BARBARIAN_ONLY) && ch->class != CLASS_BARBARIAN)
		canwear = FALSE;
...
char *res_bit_name(int res_flags)
{
	...
	if (res_flags & RES_SUMMON)
		strcat(buf, " summon");
	if (res_flags & RES_CHARM)
		strcat(buf, " charm");
	if (res_flags & RES_MAGIC)
		strcat(buf, " magic");
	if (res_flags & RES_WEAPON)
		strcat(buf, " weapon");
	if (res_flags & RES_BASH)
		strcat(buf, " blunt");
	if (res_flags & RES_PIERCE)
		strcat(buf, " piercing");
	if (res_flags & RES_SLASH)
		strcat(buf, " slashing");
	if (res_flags & RES_FIRE)
		strcat(buf, " fire");
	if (res_flags & RES_COLD)
		strcat(buf, " cold");
	if (res_flags & RES_LIGHTNING)
		strcat(buf, " lightning");
	if (res_flags & RES_ACID)
		strcat(buf, " acid");
	if (res_flags & RES_POISON)
		strcat(buf, " poison");
	if (res_flags & RES_NEGATIVE)
		strcat(buf, " negative");
	if (res_flags & RES_HOLY)
		strcat(buf, " holy");
	if (res_flags & RES_ENERGY)
		strcat(buf, " energy");
	if (res_flags & RES_MENTAL)
		strcat(buf, " mental");
	if (res_flags & RES_DISEASE)
		strcat(buf, " disease");
	if (res_flags & RES_DROWNING)
		strcat(buf, " drowning");
	if (res_flags & RES_LIGHT)
		strcat(buf, " light");
	if (res_flags & RES_IRON)
		strcat(buf, " iron");
	if (res_flags & RES_WOOD)
		strcat(buf, " wood");
	if (res_flags & RES_SILVER)
		strcat(buf, " silver");
...
char *vuln_bit_name(int vuln_flags)
{
	...
	if (vuln_flags & VULN_SUMMON)
		strcat(buf, " summon");
	if (vuln_flags & VULN_CHARM)
		strcat(buf, " charm");
	if (vuln_flags & VULN_MAGIC)
		strcat(buf, " magic");
	if (vuln_flags & VULN_WEAPON)
		strcat(buf, " weapon");
	if (vuln_flags & VULN_BASH)
		strcat(buf, " blunt");
	if (vuln_flags & VULN_PIERCE)
		strcat(buf, " piercing");
	if (vuln_flags & VULN_SLASH)
		strcat(buf, " slashing");
	if (vuln_flags & VULN_FIRE)
		strcat(buf, " fire");
	if (vuln_flags & VULN_COLD)
		strcat(buf, " cold");
	if (vuln_flags & VULN_LIGHTNING)
		strcat(buf, " lightning");
	if (vuln_flags & VULN_ACID)
		strcat(buf, " acid");
	if (vuln_flags & VULN_POISON)
		strcat(buf, " poison");
	if (vuln_flags & VULN_NEGATIVE)
		strcat(buf, " negative");
	if (vuln_flags & VULN_HOLY)
		strcat(buf, " holy");
	if (vuln_flags & VULN_ENERGY)
		strcat(buf, " energy");
	if (vuln_flags & VULN_MENTAL)
		strcat(buf, " mental");
	if (vuln_flags & VULN_DISEASE)
		strcat(buf, " disease");
	if (vuln_flags & VULN_DROWNING)
		strcat(buf, " drowning");
	if (vuln_flags & VULN_LIGHT)
		strcat(buf, " light");
	if (vuln_flags & VULN_IRON)
		strcat(buf, " iron");
	if (vuln_flags & VULN_WOOD)
		strcat(buf, " wood");
	if (vuln_flags & VULN_SILVER)
		strcat(buf, " silver");
...
char *room_affect_name(int room_flags)
{
	...
	buf[0] = '\0';
	if (room_flags & ROOM_AFF_SAFE)
		strcat(buf, " sanctuary");
	if (room_flags & ROOM_AFF_OVERGROWN)
		strcat(buf, " overgrown");
	...
	for (d = descriptor_list; d; d = d->next)
	{
		if (d->connected == CON_PLAYING)
		{
			if (IS_IMMORTAL(d->character))
				send_to_char("Send_to_world echo> ", d->character);
