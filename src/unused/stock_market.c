...
void stockargs(( CHAR_DATA *ch, CHAR_DATA *stockman ));
AREA_DATA *get_area_dataargs( ( int vnum ) );
void save_area(AREA_DATA *pArea);
void autoflux();
void bankrupt(OBJ_INDEX_DATA *obj);
int stockpercent(OBJ_INDEX_DATA *obj);

bool chance2args(( int num ));
...
void bankrupt(OBJ_INDEX_DATA *pObj)
{
	...
	if (pObj->cost != 700)
	{
		...
		sprintf(
				buf,
				"grep -rl 'Vnum %d' ../player/ | xargs perl -pi -e 's/Vnum %d/Vnum %d/'",
				pObj->vnum, pObj->vnum, 47520);//Vnum of "bad stock" object
		system(buf);
	}
...
void autoflux()
{
	...
	{
		sprintf(buf,
				"Warning: %i companies are in danger of going {DBANKRUPT!{x",
				warning);
	}
	if (warning)
		do_announce(NULL, buf);
 ...
 * it is also fundamentally unbalanced so it is commented out. 
 *
...
void do_fluct(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char(
				"Syntax: Fluct up/down/reset/now/noaesir(fluct up but not aesir)\n\r",
				ch);
		send_to_char(
				"This command will raise or lower the cost of all stocks by 15 percent\n\r",
				ch);
		return;
	}
	...
	if (!str_prefix(arg1, "up"))
	{
		...
		save_area(tarea);
		sprintf(buf,
				"The Stock Market Fluctuates! All prices went {Gup{Y 15 percent!");
		do_announce(ch, buf);
		return;
	}
	if (!str_prefix(arg1, "noaesir"))
	{
		...
		sprintf(
				buf,
				"The Stock Market Fluctuates! All prices went {Gup{Y 15 percent! Except Aesir Clan Stock.");
		do_announce(ch, buf);
		return;
	}
	else if (!str_prefix(arg1, "down"))
	{
		...
		save_area(tarea);
		sprintf(buf,
				"The Stock Market Fluctuates! All prices went {Rdown{Y 15 percent!");
		do_announce(ch, buf);
		return;
	}
	else if (!str_prefix(arg1, "reset"))
	{
		...
		save_area(tarea);
		sprintf(buf,
				"The stock prices have been {Rreset{Y to {W700{Y platinum each.");
		do_announce(ch, buf);
		return;
	}
...
void do_stockmarket(CHAR_DATA *ch, char *argument)
{
	...
	for (stockman = ch->in_room->people; stockman != NULL; stockman
			= stockman->next_in_room)
	{
		if (!IS_NPC(stockman))
			continue;
		if (stockman->spec_fun == spec_lookup("spec_stockman"))
			break;
	}
	...
	if (!strcmp(arg1, "list"))
	{
		...
		for (vnum = OBJ_VNUM_STOCK_A; vnum <= OBJ_VNUM_STOCK_E; vnum++)
		{
			strcpy(name, get_obj_index(vnum)->material);//Get the name of the stock
			if (get_obj_index(vnum)->cost != 0)
			{
				if (get_obj_index(vnum)->condition < 0
						&& get_obj_index(vnum)->condition > -10)//If it's negative and single digit
				{
					sprintf(
							listbuf,
							"    {W%i{D Plat   {W( {R%i%%{W){D:{C  %s  {D*\n\r",
							get_obj_index(vnum)->cost,
							get_obj_index(vnum)->condition, name);
					add_buf(list, listbuf);
				}
				...
				else if (get_obj_index(vnum)->condition >= 0 && get_obj_index(
						vnum)->condition < 10)//Single digit positive or zero
				{
					sprintf(
							listbuf,
							"    {W%i{D Plat   {W(  {G%i%%{W){D:{C  %s {D*\n\r",
							get_obj_index(vnum)->cost,
							get_obj_index(vnum)->condition, name);
					add_buf(list, listbuf);
					new = 2;
		...
		{
			sprintf(buf2,
					"    {y============({YImmortal {wClan Stock{y)==========={x\n\r");
			sprintf(
					buf1,
					"    {W%i{D Plat   {W({Y%i%%{W){D:{C  {GA{Ye{Ws{Gi{Yr{X Clan Stock   {D*\n\r",
					get_obj_index(OBJ_VNUM_STOCK_F)->cost, get_obj_index(
							OBJ_VNUM_STOCK_F)->condition);
		}
		else if (new == 1)
		{
			sprintf(buf2,
			...
			sprintf(
					buf1,
					"    {W%i{D Plat   {W({R-%i%%{W){D:{C  {GA{Ye{Ws{Gi{Yr{X Clan Stock   {D*\n\r",
					get_obj_index(OBJ_VNUM_STOCK_F)->cost, get_obj_index(
							OBJ_VNUM_STOCK_F)->condition);
		}
	...
	else if (!strcmp(arg1, "buy"))
	{
		...
		if (!str_prefix(arg2, "bree inc."))
		{
			...
			if (ch->platinum >= stockindex->cost)
			{
				...
				stock = create_object(get_obj_index(OBJ_VNUM_STOCK_A),
						ch->level);
			...
			else
			{
				sprintf(
						buf,
						"Sorry, %s, but you don't have enough platinum for that.",
						ch->name);
				do_say(stockman, buf);
				return;
			}
		...
		else if (!str_prefix(arg2, "fesdor unlimited"))
		{
			...
			if (ch->platinum >= stockindex->cost)
			{
				...
				stock = create_object(get_obj_index(OBJ_VNUM_STOCK_B),
						ch->level);
				obj_to_char(stock, ch);
		...
		else if (!str_prefix(arg2, "distorted illusions of grandeur"))
		{
			...
			if (ch->platinum >= stockindex->cost)
			{
				...
				stock = create_object(get_obj_index(OBJ_VNUM_STOCK_C),
						ch->level);
				obj_to_char(stock, ch);
		...
		else if (!str_prefix(arg2, "asgardian pharmaceuticals"))
		{
			...
			if (ch->platinum >= stockindex->cost)
			{
				...
				stock = create_object(get_obj_index(OBJ_VNUM_STOCK_D),
						ch->level);
				obj_to_char(stock, ch);
		...
		else if (!str_prefix(arg2, "bobs babble bonanza"))
		{
			...
			if (ch->platinum >= stockindex->cost)
			{
				...
				stock = create_object(get_obj_index(OBJ_VNUM_STOCK_E),
						ch->level);
				obj_to_char(stock, ch);
		...
		else if (!str_prefix(arg2, "Aesir Clan Stock"))
		{
			...
			if (ch->platinum >= stockindex->cost)
			{
				...
				stock = create_object(get_obj_index(OBJ_VNUM_STOCK_F),
						ch->level);
				obj_to_char(stock, ch);
