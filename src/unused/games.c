...
int Slots_Win(int slot_roll1, int slot_roll2, int slot_roll3, int bet,
		int Slots_Jackpot, OBJ_DATA *slotMachine);
...
void do_slots(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char(
				"Sorry, only player characters may use the slot machine.\n\r",
				ch);
		return;
	}
	...
	for (slotMachine = ch->in_room->contents; slotMachine != NULL; slotMachine
			= slotMachine->next_content)
	{
		if ((slotMachine->item_type == ITEM_SLOT_MACHINE) && (can_see_obj(ch,
				slotMachine)))
		{
			foundSlot = TRUE;
			break;
	...
	send_to_char(
			...
			ch);
	send_to_char(
			"{x   {3|                  {@Slots           {7-{*|{7-    {3| {7/{7/{x\n\r",
			ch);
	send_to_char(
			"{x   {3|{!> {@************************************* {!<{3|{7/{7/{x\n\r",
			ch);
	sprintf(buf, "{x   {3|{!> {@[ %s {@| %s {@| %s {@] {!<{3|{7/{x\n\r",
			bar_messages[slot_roll1], bar_messages[slot_roll2],
			bar_messages[slot_roll3]);
	send_to_char(buf, ch);
	send_to_char(
			"{x   {3|{!> {@************************************* {!<{3|{x\n\r",
			ch);
	...
	if (slot_roll1 != slot_roll2 && slot_roll1 != slot_roll3 && slot_roll2
			!= slot_roll3)
	{
		//update jackpot
		...
		sprintf(
				buf,
				"Sorry you didn't win anything. The jackpot is now worth %d silver.\n\r",
				Slots_Jackpot);
	...
	money_won = Slots_Win(slot_roll1, slot_roll2, slot_roll3, bet,
			Slots_Jackpot, slotMachine);
	...
	if (Slots_Jackpot < money_won)
	{
		...
		sprintf(
				buf,
				"The slot machine cannot cover your winnings.  You get %d silver.\n\r",
				Slots_Jackpot);
		send_to_char(buf, ch);
	...
	if ((slot_roll1 == slot_roll2) && (slot_roll2 == slot_roll3))
	{
		sprintf(
				buf,
				"You won the jackpot worth %d silver!! The jackpot now stands at %d silver.\n\r",
	...
	else
	{
		sprintf(
				buf,
				"You matched 2 bars and won %d silver! The jackpot is now worth %d silver.\n\r",
				money_won, slotMachine->value[1]);
		send_to_char(buf, ch);
	}
...
int Slots_Win(int bar1, int bar2, int bar3, int bet, int jackpot,
		OBJ_DATA *slotMachine)
{
	int winnings;
