...
void load_social_table ()
{
	...
	if (!fp)
	{
		printf_debug("Could not open " SOCIAL_FILE " for reading.");
		exit(1);
	}
...
void save_social_table()
{
	...
	if (!fp)
	{
		printf_debug("Could not open " SOCIAL_FILE " for writing.");
		return;
	}
...
void do_sedit (CHAR_DATA *ch, char *argument)
{
	...
	{
		int i,j;
		struct social_type *new_table = malloc (sizeof(struct social_type) * MAX_SOCIALS);
	...
	else if (!str_cmp(cmd, "new")) /* Create a new social */
	{
		...
		MAX_SOCIALS++;
		new_table = realloc (social_table, sizeof(struct social_type) * (MAX_SOCIALS + 1));
