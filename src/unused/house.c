...
bool change_exitargs( ( CHAR_DATA *ch, char *argument, int door ) );

void save_area(AREA_DATA *pArea);
void save_houseargs( ( ) );
void home_sell(CHAR_DATA *ch, int cost);
CHAR_DATA * find_architectargs( ( CHAR_DATA * ch ) );
HOUSE_DATA *house_list;

#define MAX_HORESET	5
#define MAX_HMRESETS	5

#define VNUM_START 5800
#define VNUM_STOP  5999

CHAR_DATA * find_architect(CHAR_DATA *ch)
{
	CHAR_DATA * architect;

	for (architect = ch->in_room->people; architect != NULL; architect
			= architect->next_in_room)
	{
		if (!IS_NPC(architect))
			continue;
	...
	if (architect == NULL || architect->spec_fun != spec_lookup(
			"spec_architect"))
	{
...
void do_objbuy(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(arg, "buy"))
	{
		...
		for (i = 0; house_table[i].name != NULL; i++)
		{
			if (is_name(arg1, house_table[i].name) && house_table[i].type
					== OBJ_VNUM)
			{
				if (ch->gold >= house_table[i].cost)
				{
					if ((obj = create_object(
							get_obj_index(house_table[i].vnum), ch->level))
							== NULL)
					{
						send_to_char(
								"That object could not be found contact an imm.\n\r",
								ch);
						return;
					}
					...
					{
						send_to_char(
								"Sorry you can only have 5 resetting objects.\n\r",
								ch);
						return;
					}
					...
					obj = create_object(get_obj_index(house_table[i].vnum),
							ch->level);
				...
				{
					printf_to_char(ch,
							"Sorry %s but you need %d gold for that.\n\r",
							ch->name, house_table[i].cost);
					return;
				}
			}
		}
		...
		for (i = 0; house_table[i].name != NULL; i++)
		{
					...
					== MOB_VNUM)
			{
				if (ch->gold >= house_table[i].cost)
				{
					...
					{
						send_to_char(
								"That mob could not be found contact an imm.\n\r",
								ch);
						return;
					}
					...
					{
						send_to_char(
								"Sorry you can only have 5 resetting mobiles.\n\r",
								ch);
						return;
					}
				...
				else
				{
					...
					printf_to_char(ch,
							"Sorry %s but you need %d gold for a %s.\n\r",
							ch->name, house_table[i].cost, house_table[i].name);
					return;
				}
			}
		}
...
void do_house(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char(
				"You already bought a house. To sell it type house <sell>.\n\r",
				ch);
	...
	if (!str_cmp(arg, "value") && HAS_HOME(ch))
	{
		...
		printf_to_char(ch,
				"You have a total of %d mobs and objects in your house.\n\r", t);
	...
	{
		send_to_char(
				"You already own a house. To sell it type house <sell>.\n\r",
				ch);
		return;
	}
	...
	while (get_room_index(i) != NULL)
	{
		...
		{
			send_to_char(
					"Sorry all of the house vnums are used up! Tell an immortal.\n\r",
					ch);
			return;
		}
	}
	...
	for (i2 = 4; i2 >= 0; i2--)
	{
		...
		{
			send_to_char(
					"Unable to create your house. Contact an Immortal.\n\r", ch);
			return;
		}
...
void do_hname(CHAR_DATA *ch, char *argument)
{
	...
	{
		printf_to_char(ch,
				"It cost %d to change the name of a room in your house.\n\r",
				cost);
		return;
	}
...
void do_home(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char(
				"Yikes write a note to immortal and let them know your house is Null.\n\r",
				ch);
		return;
	}
	...
	{
		send_to_char("Immortals have forsaken you.\n\r", ch);
		return;
	}
...
void do_invite(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char(
				"Can't invite someone to your house when you aren't in it.\n\r",
				ch);
		return;
	}
...
void home_sell(CHAR_DATA *ch, int cost)
{
	...
	printf_to_char(ch, "You recieve %d gold for selling it.\n\r", (cost / 2)
			+ value);
	printf_to_char(ch,
			"You had a total of %d mobs and objects in your house.\n\r", t);
	return;
}
...
void do_boot(CHAR_DATA *ch, char *argument)
{
	...
	if ((victim->in_room->vnum < ch->pcdata->h_vnum) || (victim->in_room->vnum
			> ch->pcdata->h_vnum + 4))
	{
	...
	printf_to_char(
			ch,
			"You get in %s's face and yell at %s to get out of your house!\n\r",
			victim->name, victim->sex == 0 ? "It" : victim->sex == 1 ? "Him"
					: "Her");
	printf_to_char(
			victim,
			"%s begins yelling at you, and tells you to get out of %s house!\n\r",
			ch->name, ch->sex == 0 ? "It's" : ch->sex == 1 ? "His" : "Her");
