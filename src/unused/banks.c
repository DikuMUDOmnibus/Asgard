...
void save_banklist(void)
{
	...
	{
		found = TRUE;
		fprintf(fp, "%d %s\n%d %d %d\n%ld\n", pbank->passwd, pbank->name,
				pbank->bank, pbank->type, pbank->amount, pbank->date);
	}

	close_write_file(BANK_FILE);
...
void do_deposit(CHAR_DATA *ch, char *argument)
{
	...
	if ((time_info.hour < bank_table[bank].open) || (time_info.hour
			> bank_table[bank].close))
	{
		sprintf(buf, "{_Sorry, our hours are %d:00%s to %d:00%s.{x",
	...
	if ((amount % 50) != 0)
	{
		send_to_char("Deposits must be made in multiples of 50 platinum.\n\r",
				ch);
		return;
	}
	...
	if ((ch->balance[bank] + amount) > 2000000000)
	{
		sprintf(
				buf,
				"%s tells you '{_Sorry, but I can only hold 2,000,000,000 coins per player.{x'\n\r",
				mob->short_descr);
		send_to_char(buf, ch);
		return;
	}
	...
	sprintf(
			buf,
			"%s tells you '{_Thank you, your new balance is {W%ld{a platinum.{x'\n\r",
...
void do_withdraw(CHAR_DATA *ch, char *argument)
{
	...
	if (((amount % 50) != 0) && (ch->balance[bank] >= 50))
	{
		send_to_char(
				"Withdrawals must be made in multiples of 50 platinum.\n\r", ch);
		return;
	}
	...
	if (amount > 50 && amount == ch->balance[bank])
		amount -= 50;
...
void do_rob(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char(
				"You need to hold someones passbook to rob thier account.\n\r",
				ch);
		return;
	}
	...
	if (found)
	{
		...
		do_give(mob, buf);
		change_banklist(ch, FALSE, bank, amount, passbook->value[1],
				passbook->name);
	}
	else
	{
		change_banklist(ch, FALSE, passbook->value[0], 0, passbook->value[1],
				passbook->name);
		sprintf(buf, "{_%s is a {RTHIEF{a!{x", ch->name);
...
void change_banklist(CHAR_DATA *ch, bool add, int bank, int amount, int pwd,
		char *argument)
{
	...
	if (!add && !str_cmp(capitalize(ch->name), capitalize(arg)))
	{
		...
		{
			if (!str_cmp(capitalize(arg), curr->name) && (curr->type == 0)
					&& (curr->bank == bank))
			{
				if (prev == NULL)
					...
					&& (curr->bank == bank))
			{
				...
				if ((victim = get_char_mortal(ch, arg)) != NULL)
				{
					if (amount > 0)
					{
						sprintf(
								buf,
								"{RSomeone just stole %d platinum from one of your accounts!{x\n\r",
								amount);
	...
	for (curr = bank_list; curr != NULL; prev = curr, curr = curr->next)
	{
		if (!str_cmp(capitalize(ch->name), curr->name))
		{
			if (curr->type == 1)
			{
				sprintf(
						buf,
						"{RSomeone stole %d platinum from one of your accounts!{x\n\r",
						curr->amount);
				send_to_char(buf, ch);
