...
void free_cln(CLN_DATA *clan)
{
	...
	{
		for (remove_mbr = clan->member_first; remove_mbr != NULL; remove_mbr
				= remove_mbr->next)
			free_mbr(remove_mbr);
...
bool remove_member(char *argument, CLN_DATA *cln)
{
	...
	else if (mbr_found->next == NULL) // Last
	{
		...
		for (mbr_find = cln->member_first; mbr_find != NULL; mbr_find
				= mbr_find->next)
		{
...
CLN_DATA *cln_lookup(char *argument)
{
	...
	{
		if (LOWER (argument[0]) == LOWER (nclan->name[0]) && !str_prefix(
				argument, nclan->name))
			return nclan;
	}
...
MBR_DATA *mbr_lookup(char *argument, CLN_DATA *clan)
{
	...
	{
		if (LOWER (argument[0]) == LOWER (nmbr->name[0]) && !str_prefix(
				argument, nmbr->name))
			return nmbr;
	}
