...
void do_oquest(CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *questman;
	...
	if (!strcmp(arg1, "string"))
	{
		if (ch->questpoints >= 300)
		{
			...
			{
				send_to_char(
						"You have to provide string to restring something.", ch);
				return;
			}
	...
	else if (!strcmp(arg1, "list"))
	{
		...
		send_to_char(
				"    {D***************({RQuest Services{D)***************{x\n\r",
				ch);
		send_to_char(
				"    {D* {cImmunities{C({W1500{C){D:{B  {r(buy hunger/thirst)     {D*\n\r",
				ch);
		send_to_char(
				"    {D* {cPretitle{C({W7500{C){D:    {r(buy pretitle {Y'{rtitle{Y'{r){D  *\n\r",
				ch);
		send_to_char(
				"    {D* {cWhotitle{C({W10000{C){D:   {r(buy whotitle {Y'{rtitle{Y'{r){D  *\n\r",
				ch);
		send_to_char(
				"    {D* {cGod Change{C({W650{C){D:   {r(buy god {Y'{rnew god{Y'{r){D     *\n\r",
				ch);
		send_to_char(
				"    {D*    {ruse {Y'{Whelp {Bnewgod{Y' {rfor this list.{D        *\n\r",
				ch);
		send_to_char(
				"    {D* {cRestring{C({W300{C){D:     {r(string {W<{robj{W><{rstring{W>{r){D  *\n\r",
				ch);
		send_to_char(
				"    {D* {cQuest Points{C({Wbuy {caquest{W/{ciquest{C){D:{r1iq {w={r 300aq{D*\n\r",
				ch);
		send_to_char(
				"    {D* {cCorpse Retrieval{C({c1 Iq{W/{C300 Aq{W/600 Plat{C){D:    *\n\r",
				ch);
		send_to_char(
				"    {D*    {r(buy retrieval {ciquest/aquest/plat{r ){D     *\n\r",
				ch);
		send_to_char(
				"    {D*                                            *\n\r", ch);
		send_to_char(
				...
				ch);
		send_to_char(
				"   {W[{wCost{W] {Y[{yKeywords{Y]               {C[{cObject{C]{x\n\r",
				ch);
		...
		for (i = 0; quest_table[i].keyword[0] != '\0'; i++)
		{
			...
			{
				sprintf(buf, "%6d %s %-25.25s %s\n\r", quest_table[i].cost,
						quest_table[i].iq ? "IQ" : "AQ",
						quest_table[i].keyword, pObj->short_descr);
				send_to_char(buf, ch);
			}
		}
	...
	else if (!strcmp(arg1, "browse"))
	{
		...
		if ((obj = create_object(get_obj_index(quest_table[i].vnum), 0))
				== NULL)
		{
	...
	else if (!strcmp(arg1, "buy"))
	{
		...
		if ((i = find_table_pos(arg2)) == -1)
		{
			if (!strcmp(arg2, "retrieval"))
			{
				...
				{
					if (obj->item_type == ITEM_CORPSE_PC && !str_cmp(
							obj->owner, ch->name))
						found = 1;
				}
				...
				if (found == 0)
				{
					sprintf(
							buf,
							"I'm sorry %s,but there is no corpse of yours out there.",
							ch->name);
					do_say(questman, buf);
				...
				{
					send_to_char(
							"{cYou need to specifiy how you want to pay for this.{x\n\r",
							ch);
					send_to_char(
							"{ySyntax{D: {cquest buy retrieval {mplatinum{w/{miquest{w/{maquest{x\n\r",
							ch);
					return;
				}
				...
				if ((is_name(arg3, "platinum") && plat < 600) || (is_name(arg3,
						"iquest") && ch->qps < 1) || (is_name(arg3, "aquest")
						&& ch->questpoints < 300))
				{
					send_to_char(
							"{cYou do not have the required amount. Either,{x\n\r",
							ch);
					send_to_char(
							"{cspecify a different payment or collect more.{x\n\r",
							ch);
					return;
				}
				...
				{
					if (obj->item_type == ITEM_CORPSE_PC && !str_cmp(
							obj->owner, ch->name) && (obj->in_room != NULL))
					{
						obj_from_room(obj);
						obj_to_char(obj, ch);
					}
				...
				sprintf(buf, "Your belongings appear in your arms!");
				do_say(questman, buf);
				return;
			}
			...
			else if (!strcmp(arg2, "god"))
			{
				...
				else
				{
					sprintf(
							buf,
							"Sorry, %s, but you don't have enough quest points to change your god.",
							ch->name);
					do_say(questman, buf);
					return;
				}
			...
			else if (!strcmp(arg2, "pretitle"))
			{
				if (ch->questpoints >= 7500)
				{
					...
					{
						send_to_char(
								"You must pick a pretitle, choose wisely..", ch);
						return;
					}
				...
				else
				{
					sprintf(
							buf,
							"Sorry, %s, but you don't have enough quest points to do that.",
							ch->name);
					do_say(questman, buf);
					return;
				}
			...
			else if (!strcmp(arg2, "whotitle"))
			{
				if (ch->questpoints >= 10000)
				{
					...
					{
						send_to_char(
								"You must choose a Whotitle, please choose Wisely.",
								ch);
						return;
					}
					cnt = 0;
			...
			else if (!strcmp(arg2, "thirst"))
			{
				if (ch->questpoints > 1500)
				{
					...
					else
					{
						...
						send_to_char(
								"Your thirst has been permanently quenched.\n\r",
								ch);
						return;
					}
				}
				else
				{
					sprintf(
							buf,
							"Sorry, %s, but you don't have enough quest points to make yourself immune to that.",
							ch->name);
					do_say(questman, buf);
					return;
				}
			...
			else if (!strcmp(arg2, "iquest"))
			{
				if (ch->questpoints < 300)
				{
					sprintf(
							buf,
							"I'm sorry %s, you don't have enough aquest points for that",
							ch->name);
					do_say(questman, buf);
					return;
				}
			...
			else if (!strcmp(arg2, "aquest"))
			{
				if (ch->qps < 1)
				{
					sprintf(
							buf,
							"I'm sorry %s, you don't have enough quest points for that",
							ch->name);
					do_say(questman, buf);
					return;
				}
		...
		if (quest_table[i].iq && ch->qps < quest_table[i].cost)
		{
			sprintf(
					buf,
					"Sorry, %s, but you don't have enough iquest points for that.",
					ch->name);
			do_say(questman, buf);
			return;
		}
		...
		if (!quest_table[i].iq && ch->questpoints < quest_table[i].cost)
		{
			sprintf(
					buf,
					"Sorry, %s, but you don't have enough aquest points for that.",
					ch->name);
			do_say(questman, buf);
			return;
		}
	...
	{
		act("$n informs $N that he is giving up on his quest.", ch, NULL,
				questman, TO_ROOM);
		act("You inform $N that you are giving up on your quest.", ch, NULL,
				questman, TO_CHAR);
	...
	else if (!strcmp(arg1, "request"))
	{
		...
		if (IS_NPC(ch))
		{
			act("$N bonks $n on the head for being a total moron.", ch, NULL,
					questman, TO_ROOM);
			act("$N bonks you on the head for being such a total moron.", ch,
		...
		if (ch->level < 15)
		{
			sprintf(buf, "Brave %s, you are too inexperienced to quest now.",
					ch->name);
			do_say(questman, buf);
		...
		if (IS_SET(ch->act, PLR_QUESTOR))
		{
			if (ch->questmob == -1 && ch->countdown > 0)
			{
				...
				if (dbl_exp || timed_dbl_exp)
				{
					...
					sprintf(
							buf,
							"As a reward, I am giving you %d quest points, and %d gold.{#(Double EXP/AQP Bonus){x",
							pointreward * 2, reward * 2);
					do_say(questman, buf);
					if (chance(3))
					{
				...
				if (obj_found == TRUE)
				{
					...
					if (dbl_exp || timed_dbl_exp)
					{
						...
						if (chance(25))
						{
							...
							sprintf(
									buf,
									"{mWith completing that quest you gained {Y%d{m experience points!",
									expreward);
							do_say(questman, buf);
						}
						if (chance(5))
						{
							ch->qps += 1;
							sprintf(buf,
									"{CWith time comes rewards, you earned 1 {YIQ{C!{x");
							do_say(questman, buf);
						}
...
void generate_quest(CHAR_DATA *ch, CHAR_DATA *questman)
{
	...
	for (mcounter = 0; mcounter < 99999; mcounter++)
	{
		...
		if ((vsearch = get_mob_index(mob_vnum)) != NULL)
		{
			...
			if (((level_diff < 70 && level_diff > -25) || (ch->level > 20
					&& ch->level < 50 && vsearch->level > 10 && vsearch->level
					< 40) || (ch->level > 50 && vsearch->level > 101))
					...
					&& !strstr(vsearch->area->builders, "Unlinked")
					&& (ch->in_room->area->continent
							== vsearch->area->continent) && chance(35))
				break;
			else
				vsearch = NULL;
		}
	...
	else
	{
		...
		case 1:
			sprintf(
					buf,
					"The Realms's most heinous criminal, %s, has escaped from the dungeon!",
					victim->short_descr);
			do_say(questman, buf);
