...
bool check_ban(char *site, int type)
{
	...
	for (pban = ban_list; pban != NULL; pban = pban->next)
	{
		...
		if (IS_SET(pban->ban_flags,BAN_PREFIX)
				&& IS_SET(pban->ban_flags,BAN_SUFFIX) && strstr(pban->name,
				host) != NULL)
			return TRUE;
		...
		if ((strstr(pban->name, host) != NULL) || !str_suffix(pban->name, host)
...
bool check_adr(char *site, int type)
{
	...
	if ((strstr(bans, host) != NULL) || !str_suffix(bans, host) || !str_prefix(
			bans, host))
		return TRUE;
