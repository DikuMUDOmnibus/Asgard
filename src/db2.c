...
void load_mobiles(FILE *fp)
{
	...
	for (;;)
	{
		...
		pMobIndex->see_all_mortals = FALSE;
		newmobs++;
		...
		for (;;)
		{
			...
			else if (letter == 'F')
			{
				...
				{
					printf_debug("Flag remove: flag not found.");
					exit(1);
				}
			}
			...
			else if (letter == 'M') /* mob prog */
			{
				...
				{
					printf_debug("MOBprogs: invalid trigger.");
					exit(1);
				}
				...
				// Mob see all
				pMobIndex->see_all_mortals = fread_number(fp);
			}
			else if (letter == 'V') {
				// Mob tier!
				pMobIndex->mob_tier = fread_number(fp);
			}
			else
			{
				ungetc(letter, fp);
...
void load_objects(FILE *fp)
{
	...
	for (;;)
	{
		...
		case ITEM_SCROLL:
			pObjIndex->value[0] = fread_number(fp);
			spell = fread_word(fp);
			if (!str_cmp(spell, "sanctuary"))
				spell = str_dup("steel flesh");
			pObjIndex->value[1] = skill_lookup(spell);
			spell = fread_word(fp);
			if (!str_cmp(spell, "sanctuary"))
				spell = str_dup("stone skin");
			pObjIndex->value[2] = skill_lookup(spell);
			spell = fread_word(fp);
			if (!str_cmp(spell, "sanctuary"))
				spell = str_dup("steel flesh");
			pObjIndex->value[3] = skill_lookup(spell);
			spell = fread_word(fp);
			if (!str_cmp(spell, "sanctuary"))
				spell = str_dup("stone skin");
			pObjIndex->value[4] = skill_lookup(spell);
			break;
		default:
		...
		for (;;)
		{
			...
			else if (letter == 'F')
			{
				...
				case 'S':
					...
					break;
				default:
					printf_debug("Load_objects: Bad where on flag set.");
					exit(1);
				}
				paf->type = -1;
				paf->level = pObjIndex->level;
			...
			else if (letter == 'G') /* guild */
			{
				if (pObjIndex->class)
				{
					printf_debug("Load_objects: duplicate class fields.");
					exit(1);
				}
			...
			for (pReset = pRoom->reset_first; pReset; pReset = pReset->next)
			{
				...
				case 'M':
					if (!(pMob = get_mob_index(pReset->arg1)))
						printf_debug("Convert_objects: 'M': bad vnum %d.", pReset->arg1);
					break;

				case 'O':
					if (!(pObj = get_obj_index(pReset->arg1)))
					{
						printf_debug("Convert_objects: 'O': bad vnum %d.", pReset->arg1);
						break;
					}
					...
					if (!pMob)
					{
						printf_debug("Convert_objects: 'O': No mob reset yet.");
						break;
					}
				...
				case 'P':
				{
					...
					if (!(pObj = get_obj_index(pReset->arg1)))
					{
						printf_debug("Convert_objects: 'P': bad vnum %d.", pReset->arg1);
						break;
					}
					...
					{
						printf_debug("Convert_objects: 'P': bad vnum %d.", pReset->arg3);
						break;
					}
				...
				case 'E':
					if (!(pObj = get_obj_index(pReset->arg1)))
					{
						printf_debug("Convert_objects: 'E' or 'G': bad vnum %d.",
								pReset->arg1);
						break;
					}
...
void convert_object(OBJ_INDEX_DATA *pObjIndex)
{
	...
	{
	default:
		printf_debug("Obj_convert: vnum %d bad type.", pObjIndex->item_type);
		break;
