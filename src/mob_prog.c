...
"objval1", "objval2", "objval3", "objval4", "grpsize", /* if grpsize $n > 6	- group size check */
"state", /* if state $n == 0 - mob state, personal to $n */
"\n" /* Table terminator */
};
...
int num_eval(int lval, int oper, int rval)
{
	...
	case EVAL_LT:
		return (lval < rval);
	default:
		printf_debug("num_eval: invalid oper");
		return 0;
	}
}
...
CHAR_DATA *get_random_char(CHAR_DATA *mob)
{
	...
	{
		if (mob != vch && !IS_NPC( vch ) && can_see(mob, vch) && (now
				= number_percent()) > highest)
		{
			victim = vch;
			highest = now;
		}
...
int count_people_room(CHAR_DATA *mob, int iFlag)
{
	...
	for (count = 0, vch = mob->in_room->people; vch; vch = vch->next_in_room)
		if (mob != vch && (iFlag == 0 || (iFlag == 1 && !IS_NPC( vch ))
				...
				&& IS_NPC( vch ) && mob->pIndexData->vnum
				== vch->pIndexData->vnum) || (iFlag == 4 && is_same_group(mob,
				vch))) && can_see(mob, vch))
			count++;
	return (count);
}
...
bool has_item(CHAR_DATA *ch, sh_int vnum, sh_int item_type, bool fWear)
{
	...
	for (obj = ch->carrying; obj; obj = obj->next_content)
		if ((vnum < 0 || obj->pIndexData->vnum == vnum) && (item_type < 0
 ...
 *----------------------------------------------------------------------
 */
int cmd_eval(sh_int vnum, char *line, int check, CHAR_DATA *mob, CHAR_DATA *ch,
		const void *arg1, const void *arg2, CHAR_DATA *rch)
{
	...
	if (rval >= 0)
	{
		...
		{
			printf_debug("Cmd_eval: prog %d syntax error(2) '%s'", vnum,
					original);
			return FALSE;
		}
	...
	if (buf[0] != '$' || buf[1] == '\0')
	{
		printf_debug("Cmd_eval: prog %d syntax error(3) '%s'", vnum, original);
		return FALSE;
	}
	else
		code = buf[1];
	...
	case 'q':
		...
		break;
	default:
		printf_debug("Cmd_eval: prog %d syntax error(4) '%s'", vnum, original);
		return FALSE;
	}
	/*
	...
	case CHK_CARRIES:
		if (is_number(buf))
			return (lval_char != NULL && has_item(lval_char, atoi(buf), -1,
					FALSE ));
		else
			return (lval_char != NULL
	...
	case CHK_HAS:
		return (lval_char != NULL && has_item(lval_char, -1, item_lookup(buf),
				FALSE ));
	case CHK_USES:
		return (lval_char != NULL && has_item(lval_char, -1, item_lookup(buf),
				TRUE ));
	...
	case CHK_POS:
		return (lval_char != NULL && lval_char->position
				== position_lookup(buf));
	case CHK_CLAN:
		return( lval_char != NULL && get_clan_by_ch(lval_char) == clan_lookup( buf ) );
	case CHK_RACE:
	...
	if ((oper = keyword_lookup(fn_evals, buf)) < 0)
	{
		printf_debug("Cmd_eval: prog %d syntax error(5): '%s'", vnum, original);
		return FALSE;
	}
	one_argument(line, buf);
	rval = atoi(buf);
	...
	case CHK_STATE:
		if (lval_char != NULL) {
			MPSTATE *mpstate;
			for (mpstate = lval_char->mpstates; mpstate != NULL; mpstate = mpstate->next)
				if (mpstate->mobvnum == (mob->pIndexData ? mob->pIndexData->vnum : -1))
					break;
			lval = mpstate ? mpstate->state : 0;
		}
		break;
	default:
		return FALSE;
		...
		const void *arg1, const void *arg2, CHAR_DATA *rch)
{
	...
	while (*str != '\0')
	{
		...
		{
		default:
			printf_debug("Expand_arg: bad code %d.", *str);
			i = " <@@@> ";
			break;
		case 'i':
		...
		case 'N':
			i
					= (ch != NULL && can_see(mob, ch)) ? (IS_NPC( ch ) ? ch->short_descr
							: get_pers(ch, NULL))
							: someone;
			break;
		case 't':
		...
		case 'T':
			i
					= (vch != NULL && can_see(mob, vch)) ? (IS_NPC( vch ) ? vch->short_descr
							: get_pers(vch, NULL))
							: someone;
			break;
		case 'r':
		...
		case 'R':
			if (rch == NULL)
				rch = get_random_char(mob);
			i
					= (rch != NULL && can_see(mob, rch)) ? (IS_NPC( ch ) ? ch->short_descr
							: get_pers(ch, NULL))
							: someone;
			break;
		case 'q':
		...
		case 'Q':
			i
					= (mob->mprog_target != NULL && can_see(mob,
							mob->mprog_target)) ? (IS_NPC( mob->mprog_target ) ? mob->mprog_target->short_descr
							: get_pers(mob->mprog_target, NULL))
							: someone;
			break;
		case 'j':
		...
		case 'e':
			i
					= (ch != NULL && can_see(mob, ch)) ? he_she[URANGE(0, ch->sex, 2)]
							: someone;
			break;
		case 'E':
			i
					= (vch != NULL && can_see(mob, vch)) ? he_she[URANGE(0, vch->sex, 2)]
							: someone;
			break;
		case 'J':
			i
					= (rch != NULL && can_see(mob, rch)) ? he_she[URANGE(0, rch->sex, 2)]
							: someone;
			break;
		case 'X':
			i
					= (mob->mprog_target != NULL && can_see(mob,
							mob->mprog_target)) ? he_she[URANGE(0, mob->mprog_target->sex, 2)]
							: someone;
			break;
		case 'k':
		...
		case 'm':
			i
					= (ch != NULL && can_see(mob, ch)) ? him_her[URANGE(0, ch ->sex, 2)]
							: someone;
			break;
		case 'M':
			i
					= (vch != NULL && can_see(mob, vch)) ? him_her[URANGE(0, vch ->sex, 2)]
							: someone;
			break;
		case 'K':
			if (rch == NULL)
				rch = get_random_char(mob);
			i
					= (rch != NULL && can_see(mob, rch)) ? him_her[URANGE(0, rch ->sex, 2)]
							: someone;
			break;
		case 'Y':
			i
					= (mob->mprog_target != NULL && can_see(mob,
							mob->mprog_target)) ? him_her[URANGE(0, mob->mprog_target->sex, 2)]
							: someone;
			break;
		case 'l':
		...
		case 's':
			i
					= (ch != NULL && can_see(mob, ch)) ? his_her[URANGE(0, ch ->sex, 2)]
							: someones;
			break;
		case 'S':
			i
					= (vch != NULL && can_see(mob, vch)) ? his_her[URANGE(0, vch ->sex, 2)]
							: someones;
			break;
		case 'L':
			if (rch == NULL)
				rch = get_random_char(mob);
			i
					= (rch != NULL && can_see(mob, rch)) ? his_her[URANGE(0, rch ->sex, 2)]
							: someones;
			break;
		case 'Z':
			i
					= (mob->mprog_target != NULL && can_see(mob,
							mob->mprog_target)) ? his_her[URANGE(0, mob->mprog_target->sex, 2)]
							: someones;
			break;
		case 'o':
			i = something;
		...
		case 'p':
			...
			break;
		case 'P':
			i = (obj2 != NULL && can_see_obj(mob, obj2)) ? obj2->short_descr
					: something;
			break;
		}
...
#define MAX_CALL_LEVEL    7 /* Maximum nested calls */
...
CHAR_DATA *mob, CHAR_DATA *ch, const void *arg1, const void *arg2)
{
	...
	{
		printf_debug("MOBprogs: MAX_CALL_LEVEL exceeded, vnum %d", mob->pIndexData->vnum);
		return;
	}
	...
	while (*code)
	{
		...
		*b = *c = *d = '\0';
		...
		if (!str_cmp(control, "if"))
		{
			...
			{
				printf_debug("Mobprog: misplaced if statement, mob %d prog %d",
						mvnum, pvnum);
				return;
			}
			...
			{
				cond[level] = cmd_eval(pvnum, line, check, mob, ch, arg1, arg2,
						rch);
			}
			else
			{
				printf_debug("Mobprog: invalid if_check (if), mob %d prog %d",
						mvnum, pvnum);
				return;
			}
		...
		else if (!str_cmp(control, "or"))
		{
			...
			{
				printf_debug("Mobprog: or without if, mob %d prog %d", mvnum,
						pvnum);
				return;
			}
			if (level && cond[level - 1] == FALSE)
			...
			{
				printf_debug("Mobprog: invalid if_check (or), mob %d prog %d",
						mvnum, pvnum);
				return;
			}
		...
		else if (!str_cmp(control, "and"))
		{
			if (!level || state[level - 1] != BEGIN_BLOCK)
			{
				printf_debug("Mobprog: and without if, mob %d prog %d", mvnum,
						pvnum);
				return;
			}
			if (level && cond[level - 1] == FALSE)
			...
			{
				printf_debug("Mobprog: invalid if_check (and), mob %d prog %d",
						mvnum, pvnum);
				return;
			}
			cond[level] = (cond[level] == TRUE) && (eval == TRUE) ? TRUE
					: FALSE;
		}
		else if (!str_cmp(control, "endif"))
		{
			if (!level || state[level - 1] != BEGIN_BLOCK)
			{
				printf_debug("Mobprog: endif without if, mob %d prog %d",
						mvnum, pvnum);
				return;
			}
			cond[level] = TRUE;
		...
		else if (!str_cmp(control, "else"))
		{
			if (!level || state[level - 1] != BEGIN_BLOCK)
			{
				printf_debug("Mobprog: else without if, mob %d prog %d", mvnum,
						pvnum);
				return;
			}
			if (level && cond[level - 1] == FALSE)
		...
		const void *arg1, const void *arg2, int type)
{
	...
	{
		if (prg->trig_type == type && strcasestr(argument, prg->trig_phrase)
				!= NULL)
		{
...
bool mp_exit_trigger(CHAR_DATA *ch, int dir)
{
	...
	{
		if (IS_NPC( mob ) && (HAS_TRIGGER(mob, TRIG_EXIT)
				|| HAS_TRIGGER(mob, TRIG_EXALL)))
		{
			for (prg = mob->pIndexData->mprogs; prg; prg = prg->next)
			{
				 ...
				 */
				if (prg->trig_type == TRIG_EXIT && dir
						== atoi(prg->trig_phrase) && mob->position
						== mob->pIndexData->default_pos && can_see(mob, ch))
				{
					program_flow(prg->vnum, prg->code, mob, ch, NULL, NULL );
					return TRUE;
				}
				else if (prg->trig_type == TRIG_EXALL && dir == atoi(
						prg->trig_phrase))
				{
					program_flow(prg->vnum, prg->code, mob, ch, NULL, NULL );
					return TRUE;
...
void mp_greet_trigger(CHAR_DATA *ch)
{
	...
	for (mob = ch->in_room->people; mob != NULL; mob = mob->next_in_room)
	{
		if (IS_NPC( mob ) && (HAS_TRIGGER(mob, TRIG_GREET)
				|| HAS_TRIGGER(mob,TRIG_GRALL)))
		{
			 ...
			 * GrAll trigger
			 */
			if (HAS_TRIGGER( mob,TRIG_GREET ) && mob->position
					== mob->pIndexData->default_pos && can_see(mob, ch))
...
void mp_hprct_trigger(CHAR_DATA *mob, CHAR_DATA *ch)
{
	...
	for (prg = mob->pIndexData->mprogs; prg != NULL; prg = prg->next)
		if ((prg->trig_type == TRIG_HPCNT) && ((100 * mob->hit / mob->max_hit)
...
char *strcasestr(char *string, char *substring)
{
	...
	ns = (char *) alloca(strlen(string)+1);

	nss = (char *) alloca(strlen(substring)+1);

	for (i = 0; i <= strlen(string); i++)
		ns[i] = tolower(string[i]);

	for (i = 0; i <= strlen(substring); i++)
		nss[i] = tolower(substring[i]);

	return strstr(ns, nss);
}
