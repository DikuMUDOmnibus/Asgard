...
void do_sing(CHAR_DATA *ch, char *argument)
{
	...
	{
		act("You attempt to sing, but begin crying instead.", ch, NULL, NULL,
				TO_CHAR);
		act("$n attempts to sing, but begins crying instead.", ch, NULL, NULL,
				TO_ROOM);
...
;		return;}
	...
	{
		default:
		printf_debug("Do_sing: bad target for sn %d.", sn );
		return;
...
/*
 void song_protection(CHAR_DATA *ch, char *argument)
 {
 AFFECT_DATA af;



 if (ch->mana < 125 || ch->move < 10)
 {
 ...
 act("A protection song consumes $n!",ch,NULL,NULL,TO_ROOM);
 send_to_char("A protection song flows within you\n\r",ch);
 improve_magic (2,ch);
...
void song_charm_person(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	for (check = char_list; check != NULL; check = check->next)
	{
		...
		{
			send_to_char(
					"You have already Charmed someone with your music.\n\r", ch);
			return;
		}
	}
