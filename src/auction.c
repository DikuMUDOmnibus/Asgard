...
extern const struct l_form look_form[];
...
BUFFER * display_stats(OBJ_DATA *obj, CHAR_DATA *ch, bool contents)
{
	...
	if (IS_IMMORTAL( ch ))
	{
		 ...
		 */
		sprintf(buf,
				"Vnum: {w%-5d       {yTimer: {w%-5d      {yResets: {w%-5d",
		...
		sprintf(
				buf,
				"{yValue[{w0{y]: {w%-7d {yValue[{w1{y]: {w%-7d {yValue[{w2{y]: {w%-7d",
		...
		sprintf(
				buf,
				"{yValue[{w3{y]: {w%-7d {yValue[{w4{y]: {w%-7d {yWeight: {w%d{y/{w%d{y/{w%d {y(10th pounds)",
				obj->value[3], obj->value[4], obj->weight, get_obj_weight(obj),
				get_true_weight(obj));
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);

		sprintf(buf, "{yMade by: {w%-26.26s {yOwner: {w%s",
				obj->maker == NULL ? "(none)" : obj->maker,
		...
		sprintf(buf, "In Room: {w%-5d    {yIn Object: {w%s", in_obj->in_room
				!= NULL ? in_obj->in_room->vnum
				: in_obj->carried_by == NULL ? 0 : in_obj->carried_by->in_room
						!= NULL ? in_obj->carried_by->in_room->vnum : 0,
				obj->in_obj == NULL ? "(none)" : obj->in_obj->short_descr);
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);

		sprintf(buf, "Wear Loc: {w%-3d     {yCarried by: {w%s", obj->wear_loc,
	...
	/*
	 ...
	 add_buf( output, final );
	 */
	sprintf(buf, "Weight: {w%d{y, Value: {w%d{y, Level: {w%d{y, Level Bonus: {w%d", obj->weight,
			obj->cost, obj->level, obj->tier_level);
	sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
	add_buf(output, final);
	/*
	 place = FALSE;
	...
	case ITEM_FURNITURE:
		sprintf(buf, "Flags: {w%s", flag_string(furniture_flags, obj->value[2]));
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);
	...
	case ITEM_CONTAINER:
	case ITEM_PIT:
		sprintf(buf, "Capacity: {w%d#{y, Maximum Weight: {w%d#", obj->value[0],
				obj->value[3]);
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);
		...
		if (obj->value[4] != 100)
		{
			sprintf(buf, "Weight Multiplier: {w%d%%", obj->value[4]);
			sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
			add_buf(output, final);
		}

		sprintf(buf, "Flags: {w%s", flag_string(container_flags, obj->value[1]));
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);
		break;

	case ITEM_WEAPON:
		sprintf(buf, "Weapon type is %s.", flag_string(weapon_class,
				obj->value[0]));
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);

		sprintf(buf, "Damage is {w%d {yd {w%d {y(average {w%d{y).",
				obj->value[1], obj->value[2], (1 + obj->value[2])
						* obj->value[1] / 2);
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);

		sprintf(buf, "Damage message: {w%s", attack_table[obj->value[3]].noun);
		sprintf(final, "| {y%s {B|\n\r", end_string(buf, 75));
		add_buf(output, final);
	...
	case ITEM_ARMOR:
		sprintf(
				buf,
				"Armor Values: Pierce: {w%d{y, Bash: {w%d{y, Slash: {w%d{y, Magic: {w%d",
	...
	if (!obj->enchanted)
	{
		...
		{
//			if (IS_IMMORTAL( ch ) || (paf->location != APPLY_NONE
//					&& paf->modifier != 0))
			{
				...
				sprintf(buf, "Affects {w%s {yby {w%d{y", flag_string(
						apply_flags, paf->location), paf->modifier);
				if (paf->location == APPLY_MORPH_FORM && paf->modifier >= 0) {
					strcat(buf, " ({w");
					strcat(buf, look_form[paf->modifier].short_descr);
					strcat(buf, "{y)");
				}
				...
				if (paf->bitvector)
				{
					...
					{
					case TO_AFFECTS:
						sprintf(final, " with {w%s {yaffect.", flag_string(
					...
					case TO_SHIELDS:
						sprintf(final, " with {w%s {yshield.", flag_string(
								shield_flags, paf->bitvector));
						break;

					case TO_RESIST:
						sprintf(final, " with {w%s {yresistance.",
								flag_string(res_flags, paf->bitvector));
						break;

					case TO_IMMUNE:
						sprintf(final, " with {w%s {yimmunity.",
								flag_string(imm_flags, paf->bitvector));
						break;

					case TO_VULN:
						sprintf(final, " with {w%s {yvulnerability.",
	...
	{
//		if (IS_IMMORTAL( ch ) || (paf->location != APPLY_NONE && paf->modifier
//				!= 0))
		{
			/*	    if ( paf->where == TO_DAM_MODS )
			 {
			...
			sprintf(buf, "Affects {w%s {yby {w%d{y", flag_string(apply_flags,
			...
			if (paf->bitvector)
			{
				...
				case TO_OBJECT:
					sprintf(final, " with {w%s {yobject flag", flag_string(
							extra_flags, paf->bitvector));
					break;

				case TO_WEAPON:
					sprintf(final, " with {w%s {yweapon flag", flag_string(
							weapon_type2, paf->bitvector));
					break;
				...
				default:
					sprintf(final, " with unknown bit {w%d{y: %d", paf->where,
							paf->bitvector);
					break;
				}
	...
	if (contents)
	{
		...
		if ((obj->item_type == ITEM_CONTAINER
				&& !IS_SET( obj->value[1], CONT_CLOSED )) || obj->item_type
...
{ "silver", "gold", "platinum", "quest points" };
...
bool can_cover_bid(CHAR_DATA *ch, int type, int amount)
{
	...
	case VALUE_QUEST_POINT:
		...
		break;
	default:
		printf_debug("can_cover_bid: invalid bid type.");
		return FALSE;
	}
...
void do_auction(CHAR_DATA *ch, char * argument)
{
	...
	{
		send_to_char(
				"You can't use the auction command with quiet or noauction set.\n\r",
				ch);
		return;
	}

	if (!str_prefix(arg1, "info") || !str_prefix(arg1, "show") || !str_prefix(
			arg1, "list") || !str_prefix(arg1, "items"))
	{
		BUFFER *final, *stats;
		...
		if (arg2[0] == '\0')
		{
			...
			add_buf(
					final,
					"{C -{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{r( {RAUCTION {r){C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C-{c={C- \n\r"
						"{C| {c*************************************************************************** {C|\n\r"
			...
			for (auc = auction_list; auc != NULL; auc = auc->next)
			{
				...
				sprintf(
						buf,
						"{C| {c* {w%2d {c* {w%s {c* {w%s {c*{w%7d {R%c{c* {w%s {c* {w%d {c* {C|\n\r",
						auc->slot, seller,
						can_see_obj(ch, auc->item) ? end_string(
								auc->item->short_descr, 26)
								: "Something                 ",
						auc->bid_amount, UPPER( *cost_string[auc->bid_type] ),
						auc->high_bidder == NULL ? "----------" : begin_string(
								PERS(auc->high_bidder,ch), 10),
		...
		else
		{
			...
			if (!can_see_obj(ch, auc->item) || (!IS_IMMORTAL( ch ) && ch
					== auc->owner))
			{
				sprintf(buf,
						"{+The current bid is {Y%d {+%s by %s{+.{x\n\r\n\r",
	...
	if (!str_cmp(arg1, "stop"))
	{
		...
		act_new("You stop the auction of $p.", ch, auc->item, NULL, TO_CHAR,
				POS_DEAD);
	...
	{
		send_to_char(
				"You do not have access to this from within a bloody cell.\n\r",
				ch);
		return;
	}
	...
	if (!str_prefix(arg1, "bid"))
	{
		...
		if (arg2[0] == '\0' || !is_number(arg2) || argument[0] == '\0'
				|| !is_number(argument))
		{
		...
		if (bid < auc->bid_amount || bid <= 0 || (auc->high_bidder != NULL
				&& bid == auc->bid_amount))
		{
			sprintf(buf,
					"{+You must bid above the current bid of {Y%d {+%s.{x\n\r",
					auc->bid_amount, cost_string[auc->bid_type]);
			send_to_char(buf, ch);
			return;
		}
	...
	if (argument[0] != '\0')
	{
		...
		{
			send_to_char(
					"Minbid must be one of: silver, gold, platinum or iquest point.\n\r",
					ch);
			return;
		}
	}
	...
	sprintf(buf,
			"{+[{Y%d{+] $N {+is now taking bids on{x $p{+. < {Y%d{+ %s >{x",
	...
	for (auc = auction_list; auc != NULL; auc = auc_next)
	{
		...
		if (++auc->status == AUCTION_LENGTH)
		{
			...
			else
			{
				...
				sprintf(buf, "You sell $p for %d %s.", auc->bid_amount,
						cost_string[auc->bid_type]);
				act_new(buf, auc->owner, auc->item, NULL, TO_CHAR, POS_DEAD );

				obj_to_char(auc->item, auc->high_bidder);

				act_new("{x$p {+appears in your hands.{x", auc->high_bidder,
						auc->item, NULL, TO_CHAR, POS_DEAD );
		...
		else
		{
			...
			else
			{
				...
				{
					sprintf(buf,
							"{x$p {+- going twice at {Y%d {+%s to $N{+.{x",
							auc->bid_amount, cost_string[auc->bid_type]);
					auction_channel(auc->high_bidder, auc->item, buf);
				}
				else if (auc->status == AUCTION_LENGTH - 2)
