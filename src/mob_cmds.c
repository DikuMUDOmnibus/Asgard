...
void mob_interpret(CHAR_DATA *ch, char *argument)
{
	...
	{
		if (command[0] == mob_cmd_table[cmd].name[0] && !str_prefix(command,
				mob_cmd_table[cmd].name))
		{
	...
	printf_debug("Mob_interpret: invalid cmd from mob %d: '%s'",
...
void do_mpstat(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(arg, "Delay   %-6d [%s]\n\r", victim->mprog_delay,
			victim->mprog_target == NULL ? "No target"
					: victim->mprog_target->name);
	send_to_char(arg, ch);
	...
	for (i = 0, mprg = victim->pIndexData->mprogs; mprg != NULL; mprg
			= mprg->next)

	{
		sprintf(arg, "[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r", ++i,
				mprog_type_to_name(mprg->trig_type), mprg->vnum,
				mprg->trig_phrase);
		send_to_char(arg, ch);
	}
...
void do_mpgecho(CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		printf_debug("MpGEcho: missing argument from vnum %d",
...
void do_mpzecho(CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		printf_debug("MpZEcho: missing argument from vnum %d",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpasound(CHAR_DATA *ch, char *argument)
{
	...
	for (door = 0; door < 6; door++)
	{
		...
		if ((pexit = was_in_room->exit[door]) != NULL && pexit->u1.to_room
				!= NULL && pexit->u1.to_room != was_in_room)
		{
...
void do_mpkill(CHAR_DATA *ch, char *argument)
{
	...
	{
		printf_debug("MpKill - Charmed mob attacking master from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpmload(CHAR_DATA *ch, char *argument)
{
	...
	{
		printf_debug("Mpmload: bad mob index (%d) from mob %d", vnum,
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpoload(CHAR_DATA *ch, char *argument)
{
	...
	if (arg1[0] != '\0')
	{
		if (!str_cmp(arg1, "randobj"))
		{
			int attempts = 0;
			while (attempts < 5 && (obj = random_object(ch->level,tier_level_bonus(ch))) == NULL)
				attempts++;
			...
			if (ch->pIndexData)
				sprintf(arg1, "Mob %d", ch->pIndexData->vnum);
			else
				sprintf(arg1, "%s", ch->name);
			obj->maker = str_dup(arg1);
			return;
		}
	}
	else if (arg1[0] == '\0' || !is_number(arg1))
	{
		printf_debug("Mpoload - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	else
	{
		...
		{
			printf_debug("Mpoload - Bad level from vnum %d.",
					IS_NPC(ch) ? ch->pIndexData->vnum : 0);
			return;
		}
	}
	...
	{
		printf_debug("Mpoload - Bad vnum arg from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mppurge(CHAR_DATA *ch, char *argument)
{
	...
	if ((victim = get_char_room(ch, arg)) == NULL)
	{
		...
		{
			printf_debug("Mppurge - Bad argument from vnum %d.",
					IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		}
		return;
	}
	...
	if (!IS_NPC( victim ))
	{
		printf_debug("Mppurge - Purging a PC from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	extract_char(victim, TRUE );
...
void do_mpgoto(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		printf_debug("Mpgoto - No argument from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	{
		printf_debug("Mpgoto - No such location from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpat(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0' || argument[0] == '\0')
	{
		printf_debug("Mpat - Bad argument from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	if ((location = find_location(ch, arg)) == NULL)
	{
		printf_debug("Mpat - No such location from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mptransfer(CHAR_DATA *ch, char *argument)
{
	...
	if (arg1[0] == '\0')
	{
		printf_debug("Mptransfer - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	else
	{
		...
		{
			printf_debug("Mptransfer - No such location from vnum %d.",
					IS_NPC(ch) ? ch->pIndexData->vnum : 0);
			return;
		}
...
void do_mpgtransfer(CHAR_DATA *ch, char *argument)
{
	...
	if (arg1[0] == '\0')
	{
		printf_debug("Mpgtransfer - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpforce(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0' || argument[0] == '\0')
	{
		printf_debug("Mpforce - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	if (!str_cmp(arg, "all"))
	{
		...
		for (vch = char_list; vch != NULL; vch = vch_next)
		{
			...
			if (vch->in_room == ch->in_room && get_trust(vch) < get_trust(ch)
					&& can_see(ch, vch))
			{
...
void do_mpgforce(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0' || argument[0] == '\0')
	{
		printf_debug("MpGforce - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpvforce(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0' || argument[0] == '\0')
	{
		printf_debug("MpVforce - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	if (!is_number(arg))
	{
		printf_debug("MpVforce - Non-number argument vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	for (victim = char_list; victim; victim = victim_next)
	{
		...
		if (IS_NPC(victim) && victim->pIndexData->vnum == vnum && ch != victim
...
void do_mpcast(CHAR_DATA *ch, char *argument)
{
	...
	if (spell[0] == '\0')
	{
		printf_debug("MpCast - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	{
		printf_debug("MpCast - No such spell from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpdamage(CHAR_DATA *ch, char *argument)
{
	...
	if (target[0] == '\0')
	{
		printf_debug("MpDamage - Bad syntax from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	{
		printf_debug("MpDamage - Bad damage min vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	if (is_number(max))
	...
	{
		printf_debug("MpDamage - Bad damage max vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	one_argument(argument, target);
	...
	if (fAll)
	{
		...
		damage(victim, victim, fKill ?
		number_range(low, high)
				: UMIN(victim->hit,number_range(low,high)), TYPE_UNDEFINED,
				DAM_NONE, FALSE, 0);
	return;
}
...
void do_mpremember(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] != '\0')
		...
		printf_debug("MpRemember: missing argument from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
}
...
void do_mpdelay(CHAR_DATA *ch, char *argument)
{
	...
	if (!is_number(arg))
	{
		printf_debug("MpDelay: invalid arg from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpcall(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		printf_debug("MpCall: missing arguments from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	if ((prg = get_mprog_index(atoi(arg))) == NULL)
	{
		printf_debug("MpCall: invalid prog from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	vch = NULL;
...
void do_mpotransfer(CHAR_DATA *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		printf_debug("MpOTransfer - Missing argument from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	one_argument(argument, buf);
	...
	{
		printf_debug("MpOTransfer - No such location from vnum %d.",
				IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
...
void do_mpremove(CHAR_DATA *ch, char *argument)
{
	...
	{
		printf_debug("MpRemove: Invalid object(%s) from vnum %d.",
...
void do_mpstate(CHAR_DATA *ch, char *argument) {
	char arg[MAX_INPUT_LENGTH];
	CHAR_DATA *victim;
	MPSTATE *mpstate;
	int state;
	...
	if (ch == victim) {
		printf_debug("MpState: victim is ch (not allowed) from vnum %d",
			ch->pIndexData->vnum);
		return;
	}
	...
	argument = one_argument(argument, arg);
	if (!is_number(arg)) {
		printf_debug("MpState: Invalid state(%s) from vnum %d",
			arg, ch->pIndexData->vnum);
		return;
	}
	state = atoi(arg);

	for (mpstate = victim->mpstates; mpstate != NULL; mpstate = mpstate->next)
		if (mpstate->mobvnum == ch->pIndexData->vnum)
			break;

	if (mpstate == NULL) {
		mpstate = new_mpstate();
		mpstate->next = victim->mpstates;
		victim->mpstates = mpstate;
		mpstate->mobvnum = ch->pIndexData->vnum;
	}
	mpstate->state = state;
}

bool mpaward_helper(char *type_arg, int amount, CHAR_DATA *victim) {
	char buf[MAX_STRING_LENGTH];
    if (!str_cmp(type_arg, "exp")) {
        gain_exp(victim, amount, FALSE, TRUE);
    } else if (!str_cmp(type_arg, "qps")) {
        sprintf(buf, "{YYou have been awarded {G%d{Y questpoints!{x\r\n", amount);
        send_to_char(buf, victim);
        victim->questpoints += amount;
    } else if (!str_cmp(type_arg, "platinum")) {
        sprintf(buf, "{YYou have been awarded {G%d{Y platinum!{x\r\n", amount);
        send_to_char(buf, victim);
        victim->platinum += amount;
    } else
        return FALSE;
...
void do_mpaward(CHAR_DATA *ch, char *argument) {
	...
	if (IS_NPC(victim)) {
		printf_debug("MpAward: victim is NPC (not allowed) from vnum %d",
			ch->pIndexData->vnum);
		return;
	}

	argument = one_argument(argument, arg);
	if (!is_number(arg) || (amount = atoi(arg)) > 10000 || amount < -10000) {
		printf_debug("MpAward: Invalid value(%s) from vnum %d",
			arg, ch->pIndexData->vnum);
		return;
	}

	argument = one_argument(argument, arg);
	if (!mpaward_helper(arg, amount, victim)) {
        printf_debug("MpAward: Invalid award type(%s) from vnum %d",
            arg, ch->pIndexData->vnum);
        return;
	}
	if (argument[0] != '\0' && !str_cmp(argument, "group") && victim->in_room != NULL) {
		CHAR_DATA *gch;
		for (gch = victim->in_room->people; gch != NULL; gch = gch->next) {
			if (gch != victim && is_same_group(gch, victim) && !IS_NPC(gch))
				mpaward_helper(arg, amount, gch);
		}
	}
}
