...
void song_update(void)
{
	...
	{
		if (channel_songs[0] >= MAX_LINES || channel_songs[0]
				>= song_table[channel_songs[1]].lines)
		{
			channel_songs[0] = -1;
	...
	for (obj = object_list; obj != NULL; obj = obj->next)
	{
		...
		if (obj->value[0] < 0)
		{
			sprintf(buf, "$p starts playing {-%s, %s{x.",
		...
		{
			if (obj->value[0] >= MAX_LINES || obj->value[0]
					>= song_table[obj->value[1]].lines)
			{
...
void load_songs(void)
{
	...
	{
		printf_debug("Couldn't open music file, no songs available.");
		fclose(fp);
		return;
	}
	...
	for (count = 0; count < MAX_SONGS; count++)
	{
		...
		for (;;)
		{
			...
			if (lines >= MAX_LINES)
			{
				printf_debug("Too many lines in a song -- limit is  %d.", MAX_LINES);
				break;
			}
...
void do_play(CHAR_DATA *ch, char *argument)
{
		...
		for (i = 0; i < MAX_SONGS; i++)
		{
			...
			if (artist
				...
				sprintf(buf, "%-39s %-39s\n\r", song_table[i].group,
						song_table[i].name);
			else if (!artist && (!match || !str_prefix(argument,
					song_table[i].name)))
	...
	if ((global && channel_songs[MAX_GLOBAL] > -1) || (!global
			&& juke->value[4] > -1))
	{
