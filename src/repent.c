...
void do_repent(CHAR_DATA *ch, char *argument)
{
	...
	{
		/* display price list */
		act("$N says '{_I offer the following services:{x'", ch, NULL, mob,
				TO_CHAR);
		send_to_char("  align: {&O{7di{&n{x's forgiveness     50 Gold\n\r", ch);
		send_to_char("  bless: {&O{7di{&n{x's blessing        50 Gold\n\r", ch);
		send_to_char("  stone skin: {&O{7di{&n{x's protection  35 gold\n\r", ch);
	...
	{
		act("$N says '{_Type 'repent' for a list of spells.{x'", ch, NULL, mob,
				TO_CHAR);
		return;
	}
	...
	{
		act("$N says '{_You do not have enough gold for my services.{x'", ch,
				NULL, mob, TO_CHAR);
		return;
	}
	...
	{
		act("$N says '{_{&O{7di{&n{a does not protect the evil at heart!{x'",
				ch, NULL, mob, TO_CHAR);
		return;
	}
...
;
	...
	if (spell == NULL) /* Increase alignment toward good */
	{
		...
		send_to_char("You feel {!H{1e{!l{x's anger as you become holier!\n\r",ch);
		return;
	}
