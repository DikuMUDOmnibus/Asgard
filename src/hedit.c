...
void hedit(CHAR_DATA *ch, char *argument)
{
	...
	if (had == NULL)
	{
		printf_debug("hedit : help for %s is NULL", pHelp->keyword);
		edit_done(ch);
		return;
	}
...
void do_hedit(CHAR_DATA *ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		...
		send_to_char(
				"If in despair...type Hedit summary, then list all once in Hedit",
				ch);
		return;
	}
...
HEDIT(hedit_delete)
{
	...
	if (help_first == pHelp) {
		...
		if (!temp)
		{
			printf_debug("hedit_delete : help %s not found in help_first",
					pHelp->keyword);
			return FALSE;
		}
	...
	if (!found)
	{
		printf_debug("hedit_delete : help %s not found in had_list", pHelp->keyword);
		return FALSE;
	}
...
HEDIT(hedit_list)
{
	...
	if (!str_cmp(argument, "area"))
	{
		...
		for (pHelp = ch->in_room->area->helps->first; pHelp; pHelp
				= pHelp->next_area)
		{
			sprintf(buf, "%3d. %-14.14s%s", cnt, pHelp->keyword,
					cnt % 4 == 3 ? "\n\r" : " ");
...

	{
		...
		if (had)
		{
			...
			{
				sprintf(buf, "%3d. %-14.14s%s", cnt, pHelp->keyword, cnt % 4
						== 3 ? "\n\r" : " ");
				add_buf(buffer, buf);
				cnt++;
			}
...
HEDIT(hedit_newfile)
{
	...
	{
		send_to_char(
				"Syntax : newfile [name] Do not attach .are or .hlp to name.\n\r",
				ch);
		return FALSE;
	}
